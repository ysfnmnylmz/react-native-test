                                    ...
                                    <View style={{ width: '100%', flex: 1, alignItems: 'center' }}>
                                        ...
                                        <View style={{ flex: 1, flexDirection: 'row', marginBottom: 25 }}>
                                            {homePrematches.map((preMatch, i) => {
                                                return (
                                                    <View style={preMatch.winningTeam === home.id ? styles.win : preMatch.winningTeam == "-1" ? styles.draw : styles.lost} key={String(i)}>
                                                        <Text style={styles.textStyle}>{preMatch.winningTeam == "-1" ? "B" : preMatch.winningTeam === home.id ? "G" : "M"}</Text>
                                                    </View>
                                                )
                                            })}
                                        </View>
                                        {homePrematches.map((prevMatch, i) => {
                                            return (
                                                <View style={styles.row} key={String(i)}>
                                                    <Image style={styles.prevMatchImage} source={{ uri: `https://cdn.footystats.org/img/${prevMatch.home_image}` }} />
                                                    <View style={styles.column}>
                                                        <Text style={styles.FtimeScore}>{`${prevMatch.homeGoalCount} - ${prevMatch.awayGoalCount}`}</Text>
                                                        <Text style={styles.HtimeScore}>{`${prevMatch.ht_goals_team_a} - ${prevMatch.ht_goals_team_b}`}</Text>
                                                    </View>
                                                    <Image style={styles.prevMatchImage} source={{ uri: `https://cdn.footystats.org/img/${prevMatch.away_image}` }} />
                                                </View>
                                            )
                                        })}
                                    </View>
                                    ...
                                    <View style={{ width: '100%', flex: 1, alignItems: 'center' }}>
                                        ...
                                        <View style={{ flex: 1, flexDirection: 'row', marginBottom: 25 }}>
                                            {awayPrematches.map((preMatch, i) => {
                                                return (<View style={preMatch.winningTeam === away.id ? styles.win : preMatch.winningTeam == "-1" ? styles.draw : styles.lost} key={String(i)}>
                                                    <Text style={styles.textStyle}>{preMatch.winningTeam == "-1" ? "B" : preMatch.winningTeam === away.id ? "G" : "M"}</Text>
                                                </View>)
                                            })}
                                        </View>
